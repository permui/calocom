...
use crate::{
    ...
    midend::middle_ir::{Block, FuncDef, VarDef, VarDefRef, Value},
    MiddleIR,
};

pub struct CodeGen<'ctx, 'module> {
    context: &'ctx Context,
    pub module: &'module mut Module<'ctx>,
    module_arena: Arena<Module<'ctx>>,
    builder: Builder<'ctx>,
    memory_layout_ctx: MemoryLayoutContext<'ctx>,
    function_value_ctx: NameContext<FunctionValue<'ctx>>,
    variable_value_ctx: HashMap<VarDef, PointerValue<'ctx>>,
}
...
impl<'ctx, 'module> CodeGen<'ctx, 'module> {
    pub fn new(name: &'ctx str, llvm_ctx: &'ctx Context, mir: &MiddleIR, path: &Path) -> Self {
        ...
        let module = module_arena.alloc(llvm_ctx.create_module(name));

        let builder = llvm_ctx.create_builder();
        let ty_ctx = mir.ty_ctx.clone();

        module.link_calocom_runtime_module(path);
        let mem_layout_ctx = MemoryLayoutContext::new(ty_ctx, llvm_ctx, module);
        ...
        CodeGen {
            ...
            variable_value_ctx: Default::default(),
            module_arena,
        }
    }
    ...
    fn emit_local_alloca(
        ...
        variables: &SlotMap<VarDefRef, VarDef>,
        variable_value_ctx: &mut HashMap<VarDef, PointerValue<'ctx>>,
        need_extra_ptr: bool,
    ) {
        ...
        for (_, var) in variables {
            ...
            variable_value_ctx.insert(var.clone(), stack_slot);
        }
    }
    ...
    fn emit_unboxed_value(&self, typed_value: &Value) -> BasicValueEnum<'ctx> {
        let Value { typ, val } = typed_value;
        match val {
            ValueEnum::ExtCall(_, _) | Value::Call(_, _) | Value::VarRef(_) => {
