...
impl<'ctx> MemoryLayoutContext<'ctx> {
    fn initialize(&mut self) {
        ...
        for (ty, (idx, llvm_ty)) in self.ty_ctx.types.iter().zip(self.llvm_types.iter_mut().enumerate()) {
            match ty {
                Type::Tuple(x) => {
    ...
    pub fn new(ty_ctx: TypeContext, llvm_ctx: &'ctx Context) -> Self {
